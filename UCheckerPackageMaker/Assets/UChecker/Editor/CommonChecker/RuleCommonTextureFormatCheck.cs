...
namespace UChecker.Editor
{
    ...
    /// 检查问题压缩格式
    /// </summary>
    [CommonCheckAttribute("检查移动平台纹理压缩格式","检查纹理压缩格式 默认压缩格式 ASTC6x6 需要其它压缩格式可添加Format多个字段",998)]
    public class RuleCommonTextureFormatCheck : BaseCommonCheck
    {
        ...
        protected override ECheckResult ForEachCheckAssetPath(string path, ConfigCell cell, ReportInfo reportInfo, out Object asset)
        {
            ...
            if (textureImporter != null)
            {
                ...
                {
                    reportInfo.AddAssetError(asset, string.Format("Android压缩格式 : {0} Android:{1}  正确格式可为:{2}", path, androidPlatformSetting.format.ToString(), formats));
                }

                if (!m_defaultFormats.Contains(iosPlatformSetting.format))
                {
                    reportInfo.AddAssetError(asset, string.Format("Android压缩格式 : {0} IOS:{1}  正确格式可为:{2}", path, androidPlatformSetting.format.ToString(), formats));
                }
